...
. Create a function at the end of the file `service/app.js` with the following code to trace every incomming http request.
+
[source, javascript]
----
...
+
This function will extract the tracing headers from the incomming http request using the function `tracer.extract(opentracing.FORMAT_HTTP_HEADERS, req.headers)`

+
With this information, if the request contains a trace information, it will be use to create a child span this way the previous service, the current service are associated int he tracing dependency graph.
...
+
The `span` is stored in the `req` object, this way the main endpoint handler can use it in case of attaching information into the same span or creating a new child span using this top level `span` as parent.
+
[source, javascript]
----
Object.assign(req, { span })
...
In the `service-a` we have the API endpoint `/sayHello`, we used this endpoint in the previous section but called it only once. This endpoint has some strange behaivor that not all responses are fast, every often the response is 100x slower. 
...
+
In the picture above, you can see a time line graph with each trace represented with a circle, in this case we have 15 traces in the result set when we clicked `Find Traces`. 
...
+
Cloud native applications can be compose of microservices and each microservice handling multiple endpoints. Having the ability to have observability allows to narrow down to a specific service, and whithin that service a specific endpoint having problems, starting with a single trace and span you can increasing the observability of your applications.

== Tracing an http handler

In the previous example we were able to identify the endpoing `/sayHello` as one of interest in our service. Lets see how can we add tracing instrumentation to the function that is handling this endpoint.
...
Notive in the Logs section the log event with the name `name` and the message `this is a log message for name Carlos`

== Tracing a function

The http handler usually call other functions to perform the business logic, when calling another function within the same service you can create a child span.
...
You can have a single trace that goes across multiple services, this allows to have distributing tracing and better observability on the interactions between services.
...
For the following example we are going to use a remote service `service-a` to format the message, and returning the formatted greeting message back to the http client.
...
. In the function `formatGreetingRemote` use the function `tracer.inject` to extract the span context and inject them into the `headers` of the http request when calling the remote servier `service-b` endpoint `/formatGreeting`.
+
[source, javascript]
----
...
If you have a specific traceid you can search for it by putting the trace id on the top left search box.

You can also use a tag to search for example searching traces that have a a specific http status code, or one of the custom tags we added to a span.
...
The Jaeger UI has a view for service depdendencies, it shows a visual Directed acyclic graph (DAG).
...
This is is a simple example and there is not much value for small set of services, but when a large number of services each with multiple endpoints then the graph becomes more interesting like the following example:
