...
. Click on one of the traces, then expand the trace's `Tags` and `Processes`. You should see information about the HTTP request such as `http.method` set to `GET` and `http.status_code` set to `200`. The Processes section shows information about the process generating the entry, such as its hostname, IP address, and the version of the Jaeger client library. 
+
